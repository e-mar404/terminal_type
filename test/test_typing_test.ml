...
let one_correct_one_incorrect_input _ = 
  let input = ref ['t'; 'b'] in
  let read_input () = 
    match !input with 
    | [] -> None
    | letter :: rest ->
  ...
  let _ = typing_test ~read_input ~display ~test_text in
  let expected_output = "\033[32mt\033[0m\033[31me\033[0m" in

  assert_equal expected_output (Buffer.contents output_buffer)

let one_incorrect_one_correct_input _ = 
  let input = ref ['a'; 'e'] in
  let read_input () = 
    match !input with 
    | [] -> None
    | letter :: rest ->
  ...
  let _ = typing_test ~read_input ~display ~test_text in
  let expected_output = "\033[31mt\033[0m\033[32me\033[0m" in
...
let suite =
  "suite" >::: [
    ...
    "one_incorrect_one_correct_input" >:: one_incorrect_one_correct_input;
  ]
